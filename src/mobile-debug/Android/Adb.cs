...
namespace VsCodeMobileUtil.Android
{
	...
	{
		public Adb(DirectoryInfo sdkHome)
		{
			AndroidSdkHome = sdkHome;
			runner = new AdbRunner(GetAdbTool(sdkHome));
		}

		public DirectoryInfo AndroidSdkHome { get; }
		
		FileInfo GetAdbTool(DirectoryInfo sdkHome)
			=> new FileInfo(Path.Combine(AndroidSdkHome.FullName, "platform-tools", "adb" + ExeSuffix));

		static string ExeSuffix => Util.IsWindows ? ".exe" : string.Empty;
